...
<%@ page import="noticeboard.*,java.text.SimpleDateFormat" %>
<!DOCTYPE HTML>
<head>
<meta charset="EUC-KR"/>
...
function deleteConfirm(num) {
	ok = confirm("삭제하시겠습니까?");
	if (ok) {
		location.href = "noticepasswordBoard.jsp?num=" + num ;
	} else {
		return;
	}			
}
		...
		<table width=700 border=0 cellspacing=0 cellpadding=7>
		 <tr><td align=center>
		 	 <table border=0>
				...
				<tr>
			     <td colspan=4>
			  		<input type=button value="수정" onClick="location.href='noticeeditBoard.jsp?num=<%=num%>'">
			    		<input type=button value="삭제" onClick="deleteConfirm(<%=num%>)"/>
					<!-- 목록보기 버튼은 listboard.jsp로 이동 -->
					<input type=button value="목록보기" onClick="location.href='noticelistBoard.jsp'">
				 </td>
			    </tr> 
		   	</table>
		  </td>
